...
export function PetitionBackPanel({
  ...
  async function backPetition() {
    ...
    const n = Math.max(1, Math.min(maxUnitsPerWallet, Number(units) || 1));
    ...
    <div className="p-4 rounded-xl border border-accent/30 bg-surface space-y-3">
      <div>
        ...
        <p className="text-[11px] text-muted mt-1 leading-relaxed">
          Each unit = {priceEth} ETH on Base. At {remainingUnits != null ? 'goal' : '1,000'} units
          sold, TMP deploys the token and airdrops fee-right units to backers. Payment stays on
          bankr.space — powered by Token Marketplace.
        </p>
      </div>

      <div className="flex flex-wrap items-end gap-3">
        <label className="text-xs text-muted">
          Units (max {cap}/wallet)
          <input
            type="number"
            min={1}
            max={cap}
        ...
        >
          {busy ? 'Confirm in wallet…' : isConnected ? 'Back with ETH' : 'Connect to back'}
        </button>
      </div>
